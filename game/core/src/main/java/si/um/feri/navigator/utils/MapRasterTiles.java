...
/**
 * Geoapify raster tiles loader (HD) with:
 * - Disk cache
 * - RAM cache
 * - Content-Type + PNG signature validation
 * - Request throttling (queue)
 * - Async loading (non-blocking render)
 *
 * Usage:
 *   Texture t = MapRasterTiles.getRasterTileCachedAsync(zoom, x, y);
 *   if (t != null) draw it; otherwise it will appear after download finishes.
 */
public class MapRasterTiles {
    ...
    // API key
    private static final String API_KEY = Keys.GEOAPIFY;
