...
bool BlockChain::addBlock(Block newBlock) {
    ...
    if (newBlock.isBlockValid(prevBlock, std::chrono::system_clock::to_time_t(std::chrono::system_clock::now()))) {
...
int BlockChain::getDifficulty() const {
    ...
    const Block &prevAdjustmentBlock = chain[chain.size() - difficultyAdjustmentInterval];
    const double timeExpected = blockGenerationInterval * difficultyAdjustmentInterval;
    const double timeTaken  = std::chrono::duration<double>(chain.back().timestamp - prevAdjustmentBlock.timestamp).count();

    int newDiff;
    if (timeTaken < timeExpected / 2.0) {
        newDiff = prevAdjustmentBlock.difficulty + 1;
        std::cout << "Difficulty is went from: " << prevAdjustmentBlock.difficulty << " to: " << newDiff << std::endl;
